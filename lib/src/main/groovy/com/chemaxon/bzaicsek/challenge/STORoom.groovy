...
class STORoom {
    static class Room {
        ...
        Room(String code) {
            ...
            for (int i = 0; i < parts.size() - 1; ++i) {
                for (int j = 0; j < parts[i].size(); ++j) {
                    ...
                    if (letterCounter.containsKey(c)) {
                        letterCounter.putAt(c, letterCounter.get(c) + 1)
                    } else {
            ...
            checkSum = matcher[0][2]
        }

        def isValid() {
            def sorted = letterCounter.entrySet().collect { new LetterFrequency(it.value, it.key) }.sort()
            if (sorted.size() < 5) {
                return false
            }
        ...
        static class LetterFrequency implements Comparable<LetterFrequency> {
            ...
            @Override
            int compareTo(LetterFrequency o) {
                def diff = o.frequency.compareTo(frequency)
                if (diff == 0) {
